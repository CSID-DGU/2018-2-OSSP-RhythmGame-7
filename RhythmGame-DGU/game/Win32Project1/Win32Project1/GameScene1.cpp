...
void GameScene1::Init()
{
	...
	if (MIX_INIT_MP3 == result)
	{
		...
		Mix_Music* music = Mix_LoadMUS("../Resources/Music/PremiumTrax-Dance With Me.mp3");
		if (NULL != music)
		{
...
void GameScene1::Update(int deltaTime)
{
...
void GameScene1::KeyDown(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
...
void GameScene1::KeyUp(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
