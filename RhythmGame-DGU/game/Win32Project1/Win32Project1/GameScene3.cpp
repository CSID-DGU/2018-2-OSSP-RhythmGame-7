...
void GameScene3::Init()
{
	...
	if (MIX_INIT_MP3 == result)
	{
		...
		Mix_Music* music = Mix_LoadMUS("../Resources/Music/Cartoon-On&On.mp3");
		if (NULL != music)
		{
...
void GameScene3::Update(int deltaTime)
{
...
void GameScene3::KeyDown(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
...
void GameScene3::KeyUp(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
