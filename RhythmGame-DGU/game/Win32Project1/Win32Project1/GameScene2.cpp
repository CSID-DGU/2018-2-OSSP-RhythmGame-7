...
void GameScene2::Init()
{
	...
	if (MIX_INIT_MP3 == result)
	{
		...
		Mix_Music* music = Mix_LoadMUS("../Resources/Music/JPB-High.mp3");
		if (NULL != music)
		{
...
void GameScene2::Update(int deltaTime)
{
...
void GameScene2::KeyDown(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
...
void GameScene2::KeyUp(unsigned int keyCode)
{
	if (TRACK_COUNT == 5)
	{
		switch (keyCode)
